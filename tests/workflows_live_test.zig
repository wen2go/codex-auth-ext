...
test "switch live action patches the current display after switching" {
    ...
    try std.testing.expectEqualStrings("Switched to Registry Beta(be*****ta@example.com)", outcome.action_message.?);
...
test "switch live action does not wait for an in-flight refresh" {
    ...
    try std.testing.expectEqualStrings("Switched to be*****ta@example.com", outcome.action_message.?);
...
test "remove live action patches the current display after deleting the active account" {
    ...
    try std.testing.expectEqualStrings("Removed 1 account(s): Registry Alpha(al*****ha@example.com)", outcome.action_message.?);
...
test "remove live action does not wait for an in-flight refresh" {
    ...
    try std.testing.expect(elapsed_ms < 500);
    try std.testing.expectEqualStrings("Removed 1 account(s): be*****ta@example.com", outcome.action_message.?);
    try std.testing.expectEqual(@as(u64, 1), runtime.display_generation);
}
